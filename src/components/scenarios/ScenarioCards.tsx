  ...
  return (
    <div className="scenario-grid">
      {summaries.map((s) => (
        <button
          key={s.scenarioId}
          className={s.scenarioId === selectedId ? 'scenario-card selected' : 'scenario-card'}
          onClick={() => onSelect(s.scenarioId)}>
          <strong>{s.scenarioId}</strong>
          <span>
            {s.claimAge === null ? 'Social Security: Already claimed' : `Claim Age: ${s.claimAge}`}
          </span>
          <span>
            Roth Conversion:{' '}
          ...
          <span>{s.depletionAge ? `Depletes around ${s.depletionAge}` : 'No modeled depletion'}</span>
        </button>
      ))}
    </div>
  );
